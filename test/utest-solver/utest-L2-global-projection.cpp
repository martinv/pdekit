...
#endif
#define BOOST_TEST_MODULE Global L2 - projection_utest
...
struct L2Global_Fixture
{
  ...
  interpolation::L2ProjectionGlobal m_projection;
...
BOOST_FIXTURE_TEST_SUITE(L2ProjectionGlobal_TestSuite, L2Global_Fixture)

// ----------------------------------------------------------------------------

BOOST_AUTO_TEST_CASE(L2_projection_global_parenv_init)
{
...
BOOST_AUTO_TEST_CASE(check_L2_projection_global_2D)
{
  ...
  const pdekit::result_of::dof_map_t<Cart2D> &source_cell_dofs = *source_dofs;
  ...
  {
    const mesh::CellTopologyView<Cart2D> tcell_view = source_cell_dofs.tcell(ActiveIdx(c));
  ...
  MeshType::dof_storage_type::clone_continuous(*mesh2D, *source_dofs, *target_dofs, P1,
  ...
  */
  ...
  m_gmsh_writer.write_mesh_to_file(*mesh2D, "source_dofs", "source_mesh_L2_global.msh");
  m_gmsh_writer.append_nodal_function_to_file(*mesh2D, "source_mesh_L2_global.msh", *solution_src,
                                              "u_source");

  m_gmsh_writer.write_mesh_to_file(*mesh2D, "target_dofs", "target_mesh_L2_global.msh");
  m_gmsh_writer.append_nodal_function_to_file(*mesh2D, "target_mesh_L2_global.msh", *solution_tgt,
                                              "u_target");
}

// ----------------------------------------------------------------------------

BOOST_AUTO_TEST_CASE(L2_projection_global_utest_parenv_finalize)
{
