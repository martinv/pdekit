...
#endif
#define BOOST_TEST_MODULE prime_basis_utest
...
BOOST_AUTO_TEST_CASE(orthonormal_prime_basis_triag_p1)
{
  point.resize(2);
  ...
  for (Uint i = 0; i < 3; ++i)
  {
    ...
    check_modal_basis<mesh::sf::DubinerExpansionTriag, mesh::utest_fixture::p1_triag_Lagrange_sf>(
        "Triag-P1-Equidist", point, 1e-14, true);
  }
}

BOOST_AUTO_TEST_CASE(orthonormal_prime_basis_triag_p2)
{
  point.resize(2);
  const Real xi0_test[3] = {-0.7, 0.7, -1.0};
  const Real xi1_test[3] = {-0.4, -0.4, 1.0};
  ...
  for (Uint i = 0; i < 3; ++i)
  {
    point[XI0] = xi0_test[i];
    point[XI1] = xi1_test[i];
    check_modal_basis<mesh::sf::DubinerExpansionTriag, mesh::utest_fixture::p2_triag_Lagrange_sf>(
        "Triag-P2-Equidist", point, 1e-14, true);
  }
}

BOOST_AUTO_TEST_CASE(orthonormal_prime_basis_triag_p3)
{
  point.resize(2);
  const Real xi0_test[3] = {-0.7, 0.7, -1.0};
  const Real xi1_test[3] = {-0.4, -0.4, 1.0};
  ...
  for (Uint i = 0; i < 3; ++i)
  {
    point[XI0] = xi0_test[i];
    point[XI1] = xi1_test[i];
    check_modal_basis<mesh::sf::DubinerExpansionTriag, mesh::utest_fixture::p3_triag_Lagrange_sf>(
        "Triag-P3-Equidist", point, 1e-14, true);
  }
}

BOOST_AUTO_TEST_CASE(orthonormal_prime_basis_tetra_p1)
{
  point.resize(3);
  const Real xi0_test[3] = {-0.7, 0.7, -1.0};
  const Real xi1_test[3] = {-0.4, -0.4, -1.0};
  const Real xi2_test[3] = {-0.1, -0.2, 1.0};
  ...
  for (Uint i = 0; i < 3; ++i)
  {
    ...
    check_modal_basis<mesh::sf::DubinerExpansionTetra, mesh::utest_fixture::p1_tetra_Lagrange_sf>(
        "Tetra-P1-Equidist", point, 1e-14, true);
  }
}

BOOST_AUTO_TEST_CASE(orthonormal_prime_basis_tetra_p2)
{
  point.resize(3);
  const Real xi0_test[3] = {-0.7, 0.7, -1.0};
  const Real xi1_test[3] = {-0.4, -0.4, -1.0};
  ...
  for (Uint i = 0; i < 3; ++i)
  {
    ...
    point[XI1] = xi1_test[i];
    point[XI2] = xi2_test[i];
    check_modal_basis<mesh::sf::DubinerExpansionTetra, mesh::utest_fixture::p2_tetra_Lagrange_sf>(
        "Tetra-P2-Equidist", point, 1e-14, true);
  }
}

BOOST_AUTO_TEST_CASE(orthonormal_prime_basis_tetra_p3)
{
  point.resize(3);
  const Real xi0_test[3] = {-0.7, 0.7, -1.0};
  const Real xi1_test[3] = {-0.4, -0.4, -1.0};
  ...
  for (Uint i = 0; i < 3; ++i)
  {
    ...
    point[XI1] = xi1_test[i];
    point[XI2] = xi2_test[i];
    check_modal_basis<mesh::sf::DubinerExpansionTetra, mesh::utest_fixture::p3_tetra_Lagrange_sf>(
        "Tetra-P3-Equidist", point, 1e-13, true);
  }
}

BOOST_AUTO_TEST_CASE(derivatives_triag)
{
  ...
  mesh::sf::DubinerExpansionTriag det;
...
BOOST_AUTO_TEST_CASE(derivatives_tetra)
{
  math::DenseDMat<Real> coordinates(3, 3);
  coordinates(0, 0) = -1.;
  coordinates(0, 1) = -1.;
  coordinates(0, 2) = -0.8;
  ...
  mesh::sf::DubinerExpansionTetra det;
  det.set_polynomial_order(P3);
  ...
  std::cout << "Derivatives with respect to xi0 = " << std::endl << dV[0] << std::endl << std::endl;
  std::cout << "Derivatives with respect to xi1 = " << std::endl << dV[1] << std::endl << std::endl;
  std::cout << "Derivatives with respect to xi2 = " << std::endl << dV[2] << std::endl << std::endl;
}
