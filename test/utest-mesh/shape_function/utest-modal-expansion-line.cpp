...
BOOST_AUTO_TEST_CASE(modal_prime_basis_line_p1)
{
  point.resize(1);
  ...
  for (Uint i = 0; i < 3; ++i)
  {
    ...
    check_modal_basis<mesh::sf::ModalExpansionLine, mesh::utest_fixture::p1_line_Lagrange_sf>(
        "Line-P1-Equidist", point, 1e-14, true);
  }
}

BOOST_AUTO_TEST_CASE(modal_prime_basis_line_p2)
{
  point.resize(1);
  const Real xi0_test[3] = {-0.7, 0.7, -1.0};
  ...
  {
    point[XI0] = xi0_test[i];
    check_modal_basis<mesh::sf::ModalExpansionLine, mesh::utest_fixture::p2_line_Lagrange_sf>(
        "Line-P2-Equidist", point, 1e-14, true);
  }
}

BOOST_AUTO_TEST_CASE(modal_prime_basis_line_p3)
{
  point.resize(1);
  const Real xi0_test[3] = {-0.7, 0.7, -1.0};
  ...
  {
    point[XI0] = xi0_test[i];
    check_modal_basis<mesh::sf::ModalExpansionLine, mesh::utest_fixture::p3_line_Lagrange_sf>(
        "Line-P3-Equidist", point, 1e-14, true);
  }
}

/*
BOOST_AUTO_TEST_CASE( derivatives_line )
{
  math::DynamicMatrix<Real> coordinates(3,1);
  coordinates(0,0) = -1.;
  coordinates(1,0) = 0.0;
  coordinates(2,0) = 0.4;

  mesh::sf::ModalExpansionLine det;
  det.set_polynomial_order(P2);

  std::vector<math::DynamicMatrix<Real> > dV;

  det.Vandermonde_matrix_derivatives(coordinates,dV);

  std::cout << "Derivatives with respect to xi0 = " << std::endl << dV[0] <<
std::endl << std::endl;
}
