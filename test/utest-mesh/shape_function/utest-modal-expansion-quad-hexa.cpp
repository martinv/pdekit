...
#endif
#define BOOST_TEST_MODULE modal_expansion_quad_hexa_utest
...
struct PrimeBasis_Fixture
{
  /// common setup for each test case
  PrimeBasis_Fixture() : ref_topology_factory(StdRegionFactory::instance())
  {
  }
...
BOOST_FIXTURE_TEST_SUITE(PrimeBasis_TestSuite, PrimeBasis_Fixture)

// ----------------------------------------------------------------------------

BOOST_AUTO_TEST_CASE(modal_prime_basis_quad_p1)
{
  point.resize(2);
  ...
  for (Uint i = 0; i < 3; ++i)
  {
    ...
    check_modal_basis<mesh::sf::ModalExpansionQuad, mesh::utest_fixture::p1_quad_Lagrange_sf>(
        "Quad-P1-Equidist", point, 1e-14, true);
  }
}

// ----------------------------------------------------------------------------

BOOST_AUTO_TEST_CASE(modal_prime_basis_quad_p2)
{
  point.resize(2);
  const Real xi0_test[3] = {-0.7, 0.7, -1.0};
  const Real xi1_test[3] = {-0.4, -0.4, 1.0};
  ...
  for (Uint i = 0; i < 3; ++i)
  {
    point[XI0] = xi0_test[i];
    point[XI1] = xi1_test[i];
    check_modal_basis<mesh::sf::ModalExpansionQuad, mesh::utest_fixture::p2_quad_Lagrange_sf>(
        "Quad-P2-Equidist", point, 1e-14, true);
  }
}

// ----------------------------------------------------------------------------

BOOST_AUTO_TEST_CASE(modal_prime_basis_quad_p3)
{
  point.resize(2);
  const Real xi0_test[4] = {-0.7, 0.7, -1.0, 0.4};
  const Real xi1_test[4] = {-0.4, -0.4, 1.0, 0.54};

  for (Uint i = 0; i < 4; ++i)
  {
    point[XI0] = xi0_test[i];
    point[XI1] = xi1_test[i];
    check_modal_basis<mesh::sf::ModalExpansionQuad, mesh::utest_fixture::p3_quad_Lagrange_sf>(
        "Quad-P3-Equidist", point, 1e-13, true);
  }
}

// ----------------------------------------------------------------------------

BOOST_AUTO_TEST_CASE(modal_prime_basis_quad_p4)
{
  point.resize(2);
  const Real xi0_test[5] = {-0.7, 0.7, -1.0, 0.4, 0.7};
  const Real xi1_test[5] = {-0.4, -0.4, 1.0, 0.54, 1.0};

  for (Uint i = 0; i < 5; ++i)
  {
    point[XI0] = xi0_test[i];
    point[XI1] = xi1_test[i];
    check_modal_basis<mesh::sf::ModalExpansionQuad, mesh::utest_fixture::p4_quad_Lagrange_sf>(
        "Quad-P4-Equidist", point, 1e-13, true);
  }
}

// ----------------------------------------------------------------------------

BOOST_AUTO_TEST_CASE(derivatives_quad)
{
  ...
  mesh::sf::ModalExpansionQuad det;
  ...
  std::cout << "Derivatives with respect to xi0 = " << std::endl
            << dV[XI0] << std::endl
            << std::endl;
  std::cout << "Derivatives with respect to xi1 = " << std::endl
            << dV[XI1] << std::endl
            << std::endl;
}
...
BOOST_AUTO_TEST_CASE(modal_prime_basis_quad_mode_degree)
{
  mesh::sf::ModalExpansionQuad mod_exp_quad;
  mod_exp_quad.set_polynomial_order(P2);

  math::DenseDVec<Uint> p_deg;
  mod_exp_quad.mode_poly_deg(p_deg);
  std::cout << "Polynomial degree of modes in expansion = " << std::endl;
  std::cout << p_deg << std::endl;
}

// ----------------------------------------------------------------------------

BOOST_AUTO_TEST_CASE(modal_prime_basis_hexa_p1)
{
  point.resize(3);
  const Real xi0_test[3] = {-0.7, 0.7, -1.0};
  const Real xi1_test[3] = {-0.4, -0.4, 1.0};
  const Real xi2_test[3] = {-0.1, 0.0, 0.65};

  for (Uint i = 0; i < 3; ++i)
  {
    point[XI0] = xi0_test[i];
    point[XI1] = xi1_test[i];
    point[XI2] = xi2_test[i];
    check_modal_basis<mesh::sf::ModalExpansionHexa, mesh::utest_fixture::p1_hexa_Lagrange_sf>(
        "Hexa-P1-Equidist", point, 1e-14, true);
  }
}

// ----------------------------------------------------------------------------

BOOST_AUTO_TEST_CASE(modal_prime_basis_hexa_p2)
{
  point.resize(3);
  const Real xi0_test[3] = {-0.7, 0.7, -1.0};
  const Real xi1_test[3] = {-0.4, -0.4, 0.93};
  const Real xi2_test[3] = {-0.1, 0.0, 0.65};
  ...
  for (Uint i = 0; i < 3; ++i)
  {
    ...
    point[XI1] = xi1_test[i];
    point[XI2] = xi2_test[i];
    check_modal_basis<mesh::sf::ModalExpansionHexa, mesh::utest_fixture::p2_hexa_Lagrange_sf>(
        "Hexa-P2-Equidist", point, 1e-14, true);
  }
}
