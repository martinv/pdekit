...
int main()
{
  ...
  MeshAdaptSequence<Cart2D> adapter;
  adapter.define_p_adapt_ops(cell_p_order);

  (*sol_dofs).adapt(adapter);
  ...
  */
  ...
  ///
  ///
  const std::string outfilename = "output_advection_rds_2D_p_adapt.msh";
  ...
  vtkwriter.append_nodal_function_to_file(*mesh2D, *sol_dofs, "output_advection_rds_2D_p_adapt.vtu",
                                          *solution, "solution");
