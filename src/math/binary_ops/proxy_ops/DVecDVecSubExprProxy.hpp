#ifndef PDEKIT_Math_DVecDVecSub_Expr_Proxy_hpp
#define PDEKIT_Math_DVecDVecSub_Expr_Proxy_hpp
...
class DVecDVecSubExprProxy : public DenseVector<DVecDVecSubExprProxy<VT1, VT2, TF>, TF>
{
  private:
  ...
  using composite_type = const DVecDVecSubExprProxy &;
  ...
  /// The matrix Subition does NOT own data in the sense that the resulting
  ...
  /// evaluate (e.g. Subition of two vectors), then Lhs type will be of type
  ...
  static_assert(!VT1::owns_data, "DVecDVecSubExprProxy can only be used with "
                                 "arguments that do not own data!");
  static_assert(!VT2::owns_data, "DVecDVecSubExprProxy can only be used with "
                                 "arguments that do not own data!");
  ...
  */
  ...
  /// Constructor
  DVecDVecSubExprProxy(const VT1 &vec_left, const VT2 &vec_right)
  ...
  friend inline void free_assign(DenseVector<VT3, TF> &lhs, const DVecDVecSubExprProxy &rhs)
  {
    DVecDVecSubExprProxy::assign(lhs.wrapped_type(), rhs.m_vec1, rhs.m_vec2);
  }
...
struct TensorRank<DVecDVecSubExprProxy<LV, RV, TF>>
{
  enum
  {
    value = tensor_rank_1
