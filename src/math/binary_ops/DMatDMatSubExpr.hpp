#ifndef PDEKIT_Math_DMat_DMat_Sub_hpp
#define PDEKIT_Math_DMat_DMat_Sub_hpp
...
class DMatDMatSubExpr : public DenseMatrix<DMatDMatSubExpr<MT1, MT2, SO>, SO>
{
  ...
  using tensor_type = typename SubTrait<MTT1, MTT2>::type;

  /// Data type for composite expression templates
  using composite_type = const DMatDMatSubExpr &;
  ...
  /// The matrix difference does NOT own data in the sense that the resulting
  ...
  /// vector or is fast to evaluate (e.g. sum of two matrices), then Lhs type
  /// will be of type 'reference to operand' Similarly for Rhs
  ...
  /// Constructor
  DMatDMatSubExpr(const MT1 &lhs, const MT2 &rhs) : m_lhs(lhs), m_rhs(rhs)
  {
  }
  ...
  friend inline void free_assign(DenseMatrix<MT3, SO> &lhs, const DMatDMatSubExpr &rhs)
  {
    DMatDMatSubExpr::assign(lhs.wrapped_type(), rhs.m_lhs, rhs.m_rhs);
  }
...
/// Operator for subtraction of two dense matrices
template <typename LM, typename RM, bool SO>
inline const DMatDMatSubExpr<LM, RM, SO> operator-(const DenseMatrix<LM, SO> &lhs,
                                                   const DenseMatrix<RM, SO> &rhs)
{
  return DMatDMatSubExpr<LM, RM, SO>(lhs.wrapped_type(), rhs.wrapped_type());
}
...
template <typename LM, typename RM, bool SO>
struct TensorRank<DMatDMatSubExpr<LM, RM, SO>>
{
  enum
  {
    value = tensor_rank_2
