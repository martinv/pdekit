#ifndef PDEKIT_Physics_Scalar_Advection3D_hpp
#define PDEKIT_Physics_Scalar_Advection3D_hpp
...
class RotationAdvection3D : public PhysModelT<Adv3DProperties>
{

  public:
  /// TYPEDEFS

  typedef typename PhysModelT<Adv3DProperties>::Properties Properties;
...
template <typename V1, typename V2, typename M1>
void RotationAdvection3D::compute_properties(const V1 &coord, const V2 &sol, const M1 &grad_vars,
                                             Properties &p)
{
...
template <typename V1>
void RotationAdvection3D::compute_variables(const Properties &p, V1 &vars)
{
  static_assert(common::IntegersAreEqual<math::TensorRank<V1>::value, math::tensor_rank_1>::value,
...
template <typename M1>
void RotationAdvection3D::flux(const Properties &p, M1 &flux)
{
  static_assert(common::IntegersAreEqual<math::TensorRank<M1>::value, math::tensor_rank_2>::value,
                "'flux' must be a matrix");
  flux(0, X) = p.V[X] * p.vars[0];
  flux(0, Y) = p.V[Y] * p.vars[0];
  flux(0, Z) = p.V[Z] * p.vars[0];
}

// ----------------------------------------------------------------------------

template <typename V1, typename V2>
void RotationAdvection3D::flux(const Properties &p, const V1 &direction, V2 &flux)
{
  static_assert(common::IntegersAreEqual<math::TensorRank<V1>::value, math::tensor_rank_1>::value,
  ...
  flux[0] = p.vars[0] * (p.V[X] * direction[X] + p.V[Y] * direction[Y] + p.V[Z] * direction[Z]);
}

// ----------------------------------------------------------------------------

template <typename V1>
inline void RotationAdvection3D::source_term(const Properties &p, V1 &src_term)
{
}
...
template <typename V1, typename V2>
void RotationAdvection3D::flux_jacobian_eigen_values(const Properties &p, const V1 &direction,
                                                     V2 &Dv)
{
  static_assert(common::IntegersAreEqual<math::TensorRank<V1>::value, math::tensor_rank_1>::value,
                "'direction' must be a vector");
  static_assert(common::IntegersAreEqual<math::TensorRank<V2>::value, math::tensor_rank_1>::value,
                "'Dv' must be a vector");
  Dv[0] = p.V[X] * direction[X] + p.V[Y] * direction[Y] + p.V[Z] * direction[Z];
}
                                                     ...
                                                     V2 &Dv, OP &op)
{
  ...
  static_assert(common::IntegersAreEqual<math::TensorRank<V2>::value, math::tensor_rank_1>::value,
                "'Dv' must be a vector");
  Dv[0] = op(p.V[X] * direction[X] + p.V[Y] * direction[Y] + p.V[Z] * direction[Z]);
}

// ----------------------------------------------------------------------------

template <typename V1, typename M1, typename M2, typename M3>
void RotationAdvection3D::flux_jacobian_eigen_structure(const Properties &p, const V1 &direction,
                                                        M1 &Rv, M2 &Lv, M3 &Dv)
{
  static_assert(common::IntegersAreEqual<math::TensorRank<V1>::value, math::tensor_rank_1>::value,
                "'direction' must be a vector");
  ...
  Rv(0, 0) = 1.;
  Lv(0, 0) = 1.;
  Dv(0, 0) = p.V[X] * direction[X] + p.V[Y] * direction[Y] + p.V[Z] * direction[Z];
}

// ----------------------------------------------------------------------------

template <typename V1, typename V2, typename M1, typename OP>
void RotationAdvection3D::build_K_mat(const Properties &p, const V1 &direction, V2 &eigvalues,
                                      M1 &K_mat, const OP &op)
{
  eigvalues[0] = p.V[X] * direction[X] + p.V[Y] * direction[Y] + p.V[Z] * direction[Z];
  K_mat(0, 0) = 1. * op(p.V[X] * direction[X] + p.V[Y] * direction[Y] + p.V[Z] * direction[Z]) * 1.;
}

// ----------------------------------------------------------------------------

template <typename M1, typename V1>
void RotationAdvection3D::residual(const Properties &p, std::array<M1, DIM> &flux_jacob, V1 &res)
{
  static_assert(common::IntegersAreEqual<math::TensorRank<M1>::value, math::tensor_rank_2>::value,
  ...
  //      y     * du/dx        +   (-x)   * du/dy
  res = A * p.grad_vars.const_col(X) + B * p.grad_vars.const_col(Y) + C * p.grad_vars.const_col(Z);
}
