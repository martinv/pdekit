#ifndef PDEKIT_RDM_Weak_Inlet_BC_hpp
#define PDEKIT_RDM_Weak_Inlet_BC_hpp
...
class WeakSubInlet
    : public WeakBC<MeshConfig, Physics, BcDim, WeakSubInlet<MeshConfig, Physics, BcDim>>
{
  private:
  using base = WeakBC<MeshConfig, Physics, BcDim, WeakSubInlet<MeshConfig, Physics, BcDim>>;
  ...
  /// Constructor
  WeakSubInlet(const std::string &name);

  /// Destructor
  ~WeakSubInlet() override;
  ...
  // Total temperature and pressure imposed at inlet
  Real m_temp_tot;
  Real m_p_tot;
};
...
WeakSubInlet<MeshConfig, Physics, BcDim>::WeakSubInlet() : base()
{
}
...
template <typename MeshConfig, typename Physics, Uint BcDim>
WeakSubInlet<MeshConfig, Physics, BcDim>::WeakSubInlet(const std::string &name) : base(name)
{
}
...
template <typename MeshConfig, typename Physics, Uint BcDim>
WeakSubInlet<MeshConfig, Physics, BcDim>::~WeakSubInlet()
{
}
...
template <typename MeshConfig, typename Physics, Uint BcDim>
void WeakSubInlet<MeshConfig, Physics, BcDim>::set_parameter(const std::string &param_name,
                                                             const Real value)
{
  if (param_name == "total_temperature")
  {
    m_temp_tot = value;
  }
  else if (param_name == "total_pressure")
  {
    m_p_tot = value;
  }
  else
  {
    std::cerr << "WeakSubInlet::set_parameter: unknown parameter name '" << param_name << "'"
              << std::endl;
  }
}
...
template <typename MeshConfig, typename Physics, Uint BcDim>
void WeakSubInlet<MeshConfig, Physics, BcDim>::compute_on_element_weak(
    ...
    sol_cell_metric_type const &cell_sol_metric, flux_cell_metric_type const &cell_flux_metric)
{
  ...
  for (Uint q = 0; q < nb_qd_pts; ++q)
  {
    ...
    const Real ghost_temp_tot =
        m_temp_tot / (1. + 0.5 * m_phys_props.gamma_minus_1 * m_phys_props.Ma * m_phys_props.Ma);

    const Real power = m_phys_props.gamma / (m_phys_props.gamma - 1.);
    const Real ghost_p_tot =
        m_p_tot /
        std::pow(1. + 0.5 * m_phys_props.gamma_minus_1 * m_phys_props.Ma * m_phys_props.Ma, power);

    m_ghost_state[0] = ghost_p_tot / (287.046 * ghost_temp_tot);
    m_ghost_state[1] = m_phys_props.Ma * std::sqrt(m_phys_props.gamma * 287.046 * ghost_temp_tot) *
                       m_ghost_state[0];

    // This needs to be fixed!
    for (Uint d = 1; d < Physics::DIM; ++d)
    {
      m_ghost_state[d + 1] = 0.0;
    }
    ...
    {
      rho2u2 += m_ghost_state[d + 1] * m_ghost_state[d + 1];
    }

    m_ghost_state[Physics::NEQ - 1] =
        ghost_p_tot / (m_phys_props.gamma_minus_1) + 0.5 * ((rho2u2) / m_ghost_state[0]);
...
template <typename MeshConfig, typename Physics, Uint BcDim>
void WeakSubInlet<MeshConfig, Physics, BcDim>::print_bc_parameters() const
{
  std::cout << "Inlet boundary condition:" << std::endl;
  std::cout << "Total temperature = " << m_temp_tot << std::endl;
  std::cout << "Total pressure    = " << m_p_tot << std::endl;
}
