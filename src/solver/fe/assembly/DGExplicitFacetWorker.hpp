#ifndef PDEKIT_DG_Explicit_Facet_Worker_hpp
#define PDEKIT_DG_Explicit_Facet_Worker_hpp
...
#include "solver/fe/facet_terms/DGFacetTerm.hpp"
...
class DGExplicitFacetWorker
{
  private:
  using facet_scheme_type = internal::DGFacetTerm<Physics>;
  ...
  void iterate_over_facets(const mesh::Tria<MeshConfig> &cell_topology,
                           ...
                           DGTimeUpdate &time_update, const Uint first_cell_idx,
  ...
  common::DataMap<mesh::PointSetTagExt, DGMethodScratchData<Physics, FACET_DATA_TOPO_DIM>>
...
template <typename MeshConfig, typename Physics, typename FacetScheme>
DGExplicitFacetWorker<MeshConfig, Physics,
                      FacetScheme>::DGExplicitFacetWorker()
    : // m_geo_cell_space(nullptr), m_geo_facet_space(nullptr),
      ...
      m_quad_order(0), m_art_viscosity(nullptr)
{
}
...
template <typename MeshConfig, typename Physics, typename FacetScheme>
DGExplicitFacetWorker<MeshConfig, Physics, FacetScheme>::~DGExplicitFacetWorker()
{
}
...
template <typename MeshConfig, typename Physics, typename FacetScheme>
void DGExplicitFacetWorker<MeshConfig, Physics, FacetScheme>::configure_cell_contour_spaces(
...
template <typename MeshConfig, typename Physics, typename FacetScheme>
void DGExplicitFacetWorker<MeshConfig, Physics, FacetScheme>::configure_facet_spaces(
    const typename f_space_facets::ptr &geo_facet_space,
    const typename f_space_facets::ptr &sol_facet_space, const SFunc sf_type,
    const PointSetID quad_type, const Uint quadrature_order)
{
  // ---------------------
  ...
  {
    common::PtrHandle<DGMethodScratchData<Physics, FACET_DATA_TOPO_DIM>> method_data_L =
        m_method_data_facet_L.create(it.key_value());
    const interpolation::FEValues &fev = (*it.data_ptr());
    (*method_data_L).resize_variables(fev);

    common::PtrHandle<DGMethodScratchData<Physics, FACET_DATA_TOPO_DIM>> method_data_R =
...
template <typename MeshConfig, typename Physics, typename FacetScheme>
void DGExplicitFacetWorker<MeshConfig, Physics, FacetScheme>::iterate_over_facets(
  ...
  */
  ...
  for (Uint bb = 0; bb < nb_buffer_blocks; ++bb)
  {
    ...
    for (Uint f = first_facet_in_block; f <= last_facet_in_block; ++f)
    {
      ...
      const std::shared_ptr<const mesh::StdRegionEntity> facet_entity_L =
          tcell_L.sub_entity(FACET_DIM, facet_blk.local_id(idx_in_block_L));
      const std::shared_ptr<const mesh::StdRegionEntity> facet_entity_R =
          tcell_R.sub_entity(FACET_DIM, facet_blk.local_id(idx_in_block_R));

      // Build discrete element key for left facet (geometry)
      const mesh::PointSetTag geo_facet_reg_L = facet_entity_L->pt_set_id();
      ...
      const mesh::PointSetTag geo_facet_reg_R = facet_entity_R->pt_set_id();
    ...
    for (Uint f = first_facet_in_block; f <= last_facet_in_block; ++f)
    {
      ...
      */
      ...
      common::PtrHandle<DGMethodScratchData<Physics, FACET_DATA_TOPO_DIM>> facet_method_data_L =
          m_method_data_facet_L.std_region_data(key_data_L);

      common::PtrHandle<DGMethodScratchData<Physics, FACET_DATA_TOPO_DIM>> facet_method_data_R =
      ...
      if (idx_in_block_L != idx_in_block_R)
      {
        ...
        for (Uint n = 0; n < sol_facet_L.nb_vert(); ++n)
        {
          ...
          {
            nodal_residual[eq] +=
                inv_node_volume * (*facet_method_data_L).m_elem_node_res[n * NEQ + eq];
          }
        }
        ...
        for (Uint n = 0; n < sol_facet_R.nb_vert(); ++n)
        {
          ...
          */
          ...
          {
            nodal_residual[eq] +=
                inv_node_volume * (*facet_method_data_R).m_elem_node_res[n * NEQ + eq];
          }
        }
