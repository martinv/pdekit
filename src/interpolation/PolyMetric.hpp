#ifndef PDEKIT_Interpolation_Poly_Metric_hpp
#define PDEKIT_Interpolation_Poly_Metric_hpp
...
class PolyMetric
{
  ...
  */
  ...
  void evaluate(GeometryMetric<MeshConfig::GDIM, MeshConfig::TDIM, DIM> const &gmetric,
                PolyCache const &sc, const ComputeMetricDerivs compute_derivatives,
  ...
  common::DataMap<mesh::DiscreteElemKey, MetricData> m_metric_data_map;
...
Uint PolyMetric<MeshConfig, DIM>::max_nb_blocks_in_buffer() const
{
  return m_nb_blocks;
}
...
template <typename MeshConfig, Uint DIM>
template <typename DiscreteElemKeyIterator>
void PolyMetric<MeshConfig, DIM>::allocate_buffer(const DiscreteElemKeyIterator keys_begin,
                                                  ...
                                                  Uint const nb_blocks)
{
  ...
  for (; it != keys_end; ++it)
  {
    ...
    {
      (*met_data).m_derivatives[d].resize(nb_qd_pts_per_elem, nb_blocks * nb_dof_per_elem);
...
template <typename MeshConfig, Uint DIM>
void PolyMetric<MeshConfig, DIM>::empty_buffer()
{
...
template <typename MeshConfig, Uint DIM>
void PolyMetric<MeshConfig, DIM>::clear()
{
  m_nb_blocks = 0;
...
template <typename MeshConfig, Uint DIM>
Uint PolyMetric<MeshConfig, DIM>::nb_values_in_buffer() const
{
...
template <typename MeshConfig, Uint DIM>
void PolyMetric<MeshConfig, DIM>::evaluate(
    GeometryMetric<MeshConfig::GDIM, MeshConfig::TDIM, DIM> const &gmetric, PolyCache const &sc,
    const ComputeMetricDerivs compute_derivatives, const RebuildMetricIndex rebuild_index)
{
  ...
  if (rebuild_index)
  {
    ...
    {
      common::PtrHandle<MetricData> md = m_metric_data_map.std_region_data(sc.key(i));
      (*md).nb_elem_filled++;
...
/*
template <typename MeshConfig, Uint DIM>
typename PolyMetric<MeshConfig, DIM>::cellwise_metric const PolyMetric<
...
template <typename MeshConfig, Uint DIM>
void PolyMetric<MeshConfig, DIM>::print_fill_status() const
{
...
template <typename MeshConfig, Uint DIM>
template <Uint Dummy>
class PolyMetric<MeshConfig, DIM>::MetricComputer<_2D, _2D, Dummy>
{
  public:
  static void compute_derivatives(GeometryMetric<_2D, _2D, DIM> const &support, PolyCache const &sc,
                                  PolyMetric<MeshConfig, DIM> &solution)
  {
    // Derivatives of one shape function in physical and reference space
    math::DenseSVec<Real, _2D> dPhysVec;
    math::DenseSVec<Real, _2D> dRefVec;

    using geo_cell_metric =
        typename GeometryMetric<MeshConfig::GDIM, MeshConfig::TDIM, DIM>::cellwise_metric;
    using mdata_type = typename PolyMetric<MeshConfig, DIM>::MetricData;
    ...
    for (typename common::DataMap<mesh::PointSetTagExt, mdata_type>::iterator it =
      ...
      // Step 1: compute derivatives in physical space by reusing the
      ...
      for (Uint c = 0; c < solution.m_mdata_index.size(); ++c)
      {
        ...
        const math::DenseDMat<Real> &dRefXI0 = data_solution.fe_values.deriv_Vandermonde(XI0);
        const math::DenseDMat<Real> &dRefXI1 = data_solution.fe_values.deriv_Vandermonde(XI1);
        ...
        const Uint nb_dof_in_elem     = solution.m_mdata_index[c].fe_values.nb_nodes();
        const Uint nb_qd_pts_per_elem = data_solution.fe_values.nb_qd_pts();
        const Uint offset_sol         = nb_dof_in_elem * solution.m_mdata_index[c].second;
        ...
        math::DenseMatView<Real> dPhysX0 = data_solution.m_derivatives[X0].block(
            0, offset_sol, nb_qd_pts_per_elem, nb_dof_in_elem);
        math::DenseMatView<Real> dPhysX1 = data_solution.m_derivatives[X1].block(
            0, offset_sol, nb_qd_pts_per_elem, nb_dof_in_elem);
        ...
        for (Uint q = 0; q < nb_qd_pts_per_elem; ++q)
        {
          ...
          for (Uint f = 0; f < solution.m_nb_fields; ++f)
          {
            ...
            */
            ...
            dRefVec[XI0] = dRefXI0(q, f);
            dRefVec[XI1] = dRefXI1(q, f);

            // dPhys = invJ * dRef;
            dPhysVec[X0] = invJ(0, 0) * dRefVec[XI0] + invJ(0, 1) * dRefVec[XI1];
            dPhysVec[X1] = invJ(1, 0) * dRefVec[XI0] + invJ(1, 1) * dRefVec[XI1];
            ...
            */
            ...
            dPhysX0(q, f) = dPhysVec[X0];
            dPhysX1(q, f) = dPhysVec[X1];
          }
...
template <typename MeshConfig, Uint DIM>
template <Uint Dummy>
class PolyMetric<MeshConfig, DIM>::MetricComputer<_3D, _3D, Dummy>
{
  public:
  static void compute_derivatives(
      GeometryMetric<MeshConfig::GDIM, MeshConfig::TDIM, DIM> const &support, PolyCache const &sc,
      PolyMetric<MeshConfig, DIM> &solution)
  {
#if 0
...
template <typename MeshConfig, Uint DIM>
template <Uint Dummy>
class PolyMetric<MeshConfig, DIM>::MetricComputer<_2D, _1D, Dummy>
{
  public:
  static void compute_derivatives(
      GeometryMetric<MeshConfig::GDIM, MeshConfig::TDIM, DIM> const &support, PolyCache const &sc,
...
template <typename MeshConfig, Uint DIM>
template <Uint Dummy>
class PolyMetric<MeshConfig, DIM>::MetricComputer<_3D, _2D, Dummy>
{
  public:
  static void compute_derivatives(
      GeometryMetric<MeshConfig::GDIM, MeshConfig::TDIM, DIM> const &support, PolyCache const &sc,
